...
6. **asset_prompts 的判据是“有没有成品图”，不是“库里有没有同名 role”**：
   凡本次需求里没有图的素材，即使能在【已有素材库】匹配到同名 role，也**必须**给
   prompt + prompt_en（匹配到 role ≠ 已有图）；只有确实已有图的才不给提词
7. 不要新增剧本以外的角色/场景/道具
...
10. **每一条 persona（旁白除外）都必须给 prompt + prompt_en**，一条都不能空
    （正面半身照：性别 / 年龄 / 发型 / 服装 / 神态 / 纯色背景）。
    缺这张参考图，出片段时同一人物会串脸、甚至整镜不出现。
---USER---
【剧本】
{{SCRIPT}}
